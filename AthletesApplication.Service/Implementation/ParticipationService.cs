...
namespace AthletesApplication.Service.Implementation
{
    public class ParticipationService : IParticipationService
    {
        ...
        {
            var participation = _participationRepository.Get(x => x, x => x.Id == id);
        ...
        public List<Athlete> GetAthletesFromUserParticipations(string userId)
        {
            return _participationRepository
                .GetAll(
                    selector: x => x.Athlete,
                    predicate: x => x.OwnerId == userId,
                    include: x => x.Include(p => p.Athlete)
                ).Distinct().ToList();
        }
    }
}
